...
def infer_ngs_groups(df_taxid, ncbi):
    ...
    if snakemake.config["ndg_options"]["groups"] != "all":
        if "NCBIGroups" not in df_taxid:
            ...
            df_taxid.loc[df_taxid.NCBIGroups.isnull(), "NCBIGroups"] = snakemake.config[
                "ndg_options"
            ]["groups"]
...
def main():
    ...
    for index, row in taxid_df.iterrows():
        try:
            ...
            tmp_df = pd.DataFrame({"TaxId": list_taxid, "NCBIGroups": row.NCBIGroups})
    ...
    for NCBIgroup, group in new_taxid_df.groupby("NCBIGroups"):
        ...
        if hard_limit < len_max:
            ...
            for index, batch in group.groupby(batch_group):
                assembly_final_df = fetch_seq_assembly(
                    batch, assembly_final_df, keyargs
                )
        else:
    ...
    assembly_final_df.iloc[:, :-1].to_csv(
        snakemake.output.assembly_output, index=False, sep="\t"
    )

    # Now handeling the last step that is concatenate the fasta files downloaded
    columns_protein_file = [
        "protein_id",
        "protein_description",
        "genome_name",
        "genome_id",
                ...
                if os.path.isfile(genome.local_filename):
                    with gzip.open(genome.local_filename, "rt") as handle:
                        ...
                        for protein in parser:
                            ...
                            protein_desciption = " ".join(
                                description_split[0].split(" ")[1:]
                            )
